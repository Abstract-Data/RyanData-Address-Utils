...
- **Definition of done:** derive from the locked language's standard checklist — for Python: `uv run pytest` + `ruff check` clean, type hints on new public functions; for TypeScript: `bun run tsc` passes with zero errors. Add project-specific criteria only if Phase 1–3 decisions imply them. On an evergreen run, only restate this if this module needs something beyond what the existing constitution already requires.
